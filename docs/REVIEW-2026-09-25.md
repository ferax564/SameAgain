...
| `npm run test:e2e` (Playwright, desktop + mobile) | 20 pass | 22 pass, including a new health-score scenario with an axe scan |
...
- `lib/retailers.ts` still lists only Coop and Migros for Switzerland. Denner, Lidl and Aldi products are indexed, but they cannot be chosen as a shop in the store hub or for offers.
- The in-app guide (`app/guide/guide.json`) is an older copy of the README and does not describe the health score or the new catalogue.
- The export snapshot must be refreshed by running `scripts/import-off-dump.py`; no scheduled refresh exists.
- Receipt-product linking has unit and route tests but no Playwright scenario yet.
