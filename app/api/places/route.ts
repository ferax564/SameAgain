...
export async function GET(req: Request) {
  try {
    ...
    await rate('places:user:' + user.id, 8);
    ...
    const base = (env as any).PHOTON_BASE_URL || 'https://photon.komoot.io/';
    ...
    await rate('photon:daily', 300, 86400000);
    ...
    const rows = places(await r.json(), at),
      data = {
        places: rows,
