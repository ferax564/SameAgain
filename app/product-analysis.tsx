...
export function ProductAnalysis({product,constraints=[]}:{product:Product;constraints?:Constraint[]}){
 ...
 return <div className="stack product-analysis">
  ...
  <section>
   ...
   {a.missingIngredients?<p className="notice">Ingredients are not in this snapshot. Open the product source or check the pack. Demo browse uses the saved catalogue; a signed-in household can refresh a barcode from Open Food Facts.</p>:<div className="ingredient-chips">{a.chips.map((c,i)=><span key={i} className={'ingredient-chip '+(c.additive?'concern-'+c.additive.concern:c.unknownCode?'concern-unknown':'')}>{c.text}{c.additive&&<em>{c.additive.id.toUpperCase()}</em>}</span>)}</div>}
