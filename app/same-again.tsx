...
import { Brand, Choice, Modal, Photo } from './ui';
import ReceiptScanner from './receipt-scanner';
import Scanner from './scanner';
import StoreHub from './store-hub';
import Meals from './meals';
import PhotoUpload from './photo-upload';
import { ProductNutrition, NutritionFields, IngredientReview } from './nutrition-panel';
...
const units = { pack: 'packs', piece: 'pieces', kg: 'kg', g: 'g', l: 'litres', ml: 'ml' };
const navigation = [
...
export default function SameAgain() {
  ...
  const [view, setView] = useState('lists'),
    ...
    [destination, setDestination] = useState('FR'),
    ...
    [decisions, setDecisions] = useState<Record<string, any>>({}),
    [inviteUrl, setInviteUrl] = useState(''),
    [confirm, setConfirm] = useState<any>(null),
    [mergeQueue, setMergeQueue] = useState<any[]>([]),
    [location, setLocation] = useState(''),
    [awake, setAwake] = useState(false);
  const wake = useRef<any>(null),
    queryTimer = useRef<any>(null),
    matchSerial = useRef(0),
    productSerial = useRef(0),
    searchSerial = useRef(0);
  const household = s.household,
    hid = household?.id;
  const merge = mergeQueue[0];
  function setMerge(v: any) {
    setMergeQueue((q) => (v ? [...q, v] : q.slice(1)));
  }
  const lists = s.records.filter((r) => r.kind === 'list' && !r.data.archived),
    active = lists.find((l) => l.id === selected) || lists[0],
    items = s.records
      .filter((r) => r.kind === 'item' && r.data.list === active?.id)
    ...
    favourites = s.records.filter((r) => r.kind === 'favourite'),
    trips = s.records.filter((r) => r.kind === 'trip').sort((a, b) => b.created - a.created),
    templates = s.records.filter((r) => r.kind === 'template');
  const categories = household?.settings?.categories || [
    'Fruit & vegetables',
    'Dairy & alternatives',
    'Bakery',
    'Pantry',
  ...
  useEffect(() => {
    const d = localStorage.getItem('same-again:theme') === 'dark';
    setDark(d);
  ...
  useEffect(() => {
    if (active && !lists.some((l) => l.id === selected)) setSelected(active.id);
  }, [hid, active?.id, selected]);
  const changeTheme = () => {
    setDark(!dark);
  ...
  function openItem(data: any = {}, old?: RecordData) {
    if (!active) {
  ...
  function add(data: any, to = active?.id, repeat = false) {
    ...
    const duplicate = s
      .currentRecords()
      .find((r) => r.kind === 'item' && r.data.list === to && !r.data.done && canMerge(r.data, d));
    if (duplicate) {
      setMerge({ existing: duplicate, data: d });
      return;
    }
    s.mutate('item', d);
    toast.success('Added to your list');
  }
  function addReceipt(rows: any[], to: string) {
  ...
  function saveItem(e: React.FormEvent) {
    ...
    if (editing) {
      ...
      if (
        ...
        draft.price = null;
        draft.actualPrice = null;
        draft.priceCurrency = newList?.data.currency;
        toast('Prices cleared because the destination list uses another currency.');
      }
      s.mutate('item', { ...draft, priceCurrency: newList?.data.currency || currency }, editing);
  ...
  function saveFavourite(data: any) {
    if (!data.product?.id) return toast.error('Save an exact or private product as a favourite.');
    if (favourites.some((f) => f.data.product?.id === data.product.id))
  ...
  async function openProduct(p: Product, store = '') {
    ...
    try {
      ...
      if (serial === productSerial.current)
        setProductNotice(
          (e.message || 'Details could not be refreshed.') +
  ...
  async function findMatches(item: RecordData) {
    ...
    try {
      ...
      if (serial === matchSerial.current) setMatchReason(e.message);
    } finally {
  ...
  function completeTrip() {
    ...
    s.mutate('trip', {
      ...
      list: active?.id,
      currency,
      items: done.map((i) => ({ ...i.data, originalItem: i.id, originalVersion: i.version })),
    });
  ...
  function renderItem(item: RecordData) {
    const d = item.data;
    return (
      <article key={item.id} className={'item-row ' + (d.done ? 'completed' : '')}>
        <Checkbox
  ...
  return (
    <SidebarProvider>
      ...
      <Sidebar collapsible="none" className="app-side">
        <SidebarContent>
          <Brand />
            ...
            <div className="row between">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setModal(household ? 'list' : 'create');
                }}
              >
                <Plus size={18} />
              </button>
      ...
      <div className={'app-body ' + (shopping ? 'shopping-mode' : '')}>
        ...
        {s.rejected && (
          <div className="notice" role="alert">
            <strong>
              A change needs attention: {s.rejected.op.data.name || s.rejected.op.kind}
            </strong>
            <p>
              {s.rejected.message}. Edit this record in the app to correct it, or discard this draft
              and any pending changes that depend on it.
            </p>
          ...
          {s.loading ? (
            ...
            <div className="onboarding">
              ...
              <div className="welcome-products">
                {demoProducts.slice(1, 4).map((p) => (
                  <div key={p.id}>
                    <Photo product={p} large />
                    <span>{p.brand?.split(',')[0]}</span>
                  </div>
                ))}
              </div>
              <p className="fine">
                Personal households are private. Only invited members can access them.
              </p>
            </div>
          ) : (
            <>
                  ...
                  <div className="lists-layout">
                        ...
                        <>
                          <form
                            ...
                            onSubmit={(e) => {
                              e.preventDefault();
                              if (quick.trim()) {
                                add({ name: quick.trim(), unit: 'piece', quantity: 1 });
                                setQuick('');
                              }
                            }}
                          >
                            ...
                            <div className="row">
                              ...
                              <div className="avatar-group">
                                {s.members.slice(0, 3).map((m) => (
                                  <span className="avatar small" key={m.user} title={m.name}>
                                    {m.name[0]}
                                  </span>
                                ))}
                              </div>
                              ...
                              <DropdownMenuContent align="end">
                                <DropdownMenuItem
                                  onClick={() => {
                                    setDraft({
                                      ...active.data,
                                      categoryOrder: (active.data.categoryOrder || categories).join(
                                        ', ',
                                      ),
                                    });
                                    setModal('listSettings');
                                  }}
                                >
                          ...
                          {outstanding.length === 0 ? (
                            ...
                            [
                              ...new Set([
                                ...listCats,
                                ...outstanding.map((i) => i.data.category || 'Other'),
                              ]),
                            ].map((category: any) => {
                          ...
                          <div className="list-footer">
                            <span>
                              <Users size={15} /> Added-by and purchase details are saved with each
                              item.
                            </span>
                          </div>
                        </>
                      ...
                      {!active && (
                        <div className="empty">
                          ...
                          <button
                            className="btn primary"
                            onClick={() => {
                              setDraft({ name: 'Weekly groceries', currency });
                              setModal('list');
                            }}
                          >
                            Create a list
                    ...
                    <aside className="context-column">
                      <section className="travel-card">
                        ...
                        <p>
                          Find your usual products—or a thoughtful alternative—in another country.
                        </p>
                        <Choice
                        ...
                        {favourites.slice(0, 3).map((f) => (
                          <div className="usual" key={f.id}>
                            ...
                            <div>
                              ...
                              <small className="muted">
                                {f.data.product?.pack || 'Household favourite'}
                              </small>
                            </div>
                            <button
                              className="iconbtn"
                      ...
                      <section className="cost-card">
                        ...
                        <p className="muted">
                          {money.missing} outstanding{' '}
                          {money.missing === 1 ? 'item has' : 'items have'} no price. This is a
                          partial estimate.
                        </p>
                        {done.length > 0 && (
                          <p className="fine">
                ...
                <>
                  ...
                  <Tabs defaultValue="products">
                    ...
                    <TabsContent value="products">
                            ...
                            setResults(
                              demoProducts.filter(
                                (p) =>
                                  ...
                                  (!searchCountry ||
                                    p.countries.includes(countryTag(searchCountry))) &&
                      ...
                      <p className="fine">
                        Search runs when you press Search. Results are cached to respect Open Food
                      ...
                      <div className="row between section-title">
                        ...
                        <span className="pill">
                          {s.demo ? 'Demo catalogue' : 'Source-labelled catalogue'}
                        </span>
                      </div>
                      {searched && !results.length && !busy ? (
                        <div className="empty card">
                          <Search size={36} />
                          <h3>
                            {searchError ? 'No cached matches available.' : 'No products found.'}
                          </h3>
                          <p>
                            Try a brand or barcode. Household essentials can always be added
                            manually.
                          </p>
                      ...
                      <p className="source-note">
                        ...
                        · ODbL 1.0. Its product images: CC BY-SA 3.0. Retailer imports have separate
                        page sources; they are not licensed under ODbL.{' '}
                    ...
                    <TabsContent value="stores">
                      ...
                      <div className="section-title">
                        ...
                        <p className="muted">
                          Dated member reports, not verified stock or current prices.
                        </p>
                      </div>
                      {s.records.filter((r) => r.kind === 'observation').length === 0 ? (
                        <div className="empty card">
                          Record where you bought an item from its list menu.
                        </div>
                      ) : (
                        s.records
                            ...
                            <article className="card observation" key={r.id}>
                              ...
                              <div>
                                ...
                                <small className="muted">
                                  ...
                                  {r.data.price !== '' && r.data.price != null
              ...
              {view === 'favourites' && (
                <>
                      ...
                      {favourites.map((f) => (
                        <article className="product-card favourite-card" key={f.id}>
                          <button
                            ...
                            onClick={() => void openProduct(f.data.product)}
                          >
                            <Photo
                              product={{
                                ...f.data.product,
                  ...
                  <section className="section-title">
                    ...
                    <p className="muted">
                      Substitutes your household approved for a specific country.
                    </p>
                  </section>
                  <div className="grid2">
                    {s.records
                ...
                <>
                  <div className="page-heading">
                    <div>
                      ...
                      <p className="muted">
                        Shared favourites. Individual tastes. Everyone has a place.
                      </p>
                    </div>
                    <button
                      className="btn primary"
                  ...
                  <div className="household-grid">
                    <section className="card">
                      ...
                      <p className="fine">
                        An invite is single-use, expires in 7 days and can be revoked. The person
                        joining signs in with their own ChatGPT account.
                      </p>
                      {s.invites
                        ...
                        .map((i) => (
                          <div className="row between invitation" key={i.id}>
                            <span>
                              {i.recipient_email || 'Invitation'} ·{' '}
                              {i.expires < Date.now() ? 'Expired' : 'Pending'} ·{' '}
                    ...
                    <section className="card">
                      ...
                      <p className="muted">
                        Export your account and shared household records. If you leave, the
                        household keeps its shopping history.
                      </p>
                      <div className="row wrap">
                        <a
                          className="btn"
        ...
        <form className="formgrid" onSubmit={saveItem}>
          ...
          <details className="item-photo-editor full">
            ...
            <PhotoUpload
              ...
              demo={s.demo}
              value={draft.image}
              onChange={(image) => setDraft((d: any) => ({ ...d, image }))}
              onBusy={setPhotoBusy}
            />
            <p className="fine">
          ...
          <label>
            Estimated price per unit ({currency})
            <input
              type="number"
              step="0.01"
              min="0"
          ...
          <label>
            Actual price per unit ({currency})
            <input
              type="number"
              step="0.01"
              min="0"
          ...
          {editing && (
            <p className="fine full">
              Added by {memberName(editing.createdBy)} ·{' '}
              {new Date(editing.created).toLocaleString()}. Last updated{' '}
              {new Date(editing.updated).toLocaleString()}.
            </p>
          )}
        ...
        {product && (
          <div className="stack">
            ...
            {product.barcode && barcode(product.barcode).local && (
              <p className="notice">
                This may be a retailer-specific or variable-weight code. Confirm the product and
                store context.
              </p>
            )}
            ...
            {s.records.some((r) => r.kind === 'product' && r.id === product.id) && (
              <button
                ...
                onClick={() => {
                  ...
                  setDraft({
                    ...r.data,
                    allergens: r.data.allergens?.join(', ') || '',
                    traces: r.data.traces?.join(', ') || '',
                  });
                  setModal('private');
                }}
              >
            ...
            <section>
              ...
              <div className="row wrap">
                {['Like', 'Dislike', 'Would buy again', 'Not a suitable substitute'].map((v) => (
                  <button
                    ...
                    className="btn"
                    onClick={() => {
                      setDraft({ product: product.id, feedback: v, reason: '' });
                      setModal('feedback');
                    }}
                  >
                    {v}
              ...
              {s.records
                .filter((r) => r.kind === 'feedback' && r.data.product === product.id)
                .map((r) => (
          ...
          <div className="grid2">
            ...
            <label>
              ...
              <input
                maxLength={500}
                value={draft.traces || ''}
                onChange={(e) => setDraft({ ...draft, traces: e.target.value })}
              />
            </label>
          </div>
          <Choice
            ...
            <>
                    ...
                    >
                      <label>
                        ...
                        <input
                          ...
                          step="any"
                          required
                          defaultValue={compareItem.data.quantity}
                        />
                      </label>
              ...
              <div className="row wrap">
                <button
                  className="btn"
                  onClick={() =>
                    choose(compareItem, compareItem.data.product, 'Kept the original product')
                  }
                >
                  Keep original
                </button>
        ...
        >
          ...
          {modal === 'create' && (
            <>
                ...
                <Choice
                  ...
                  onChange={(v) =>
                    setDraft({
                      ...
                      country: v,
                      currency:
                        v === 'US' ? 'USD' : v === 'GB' ? 'GBP' : v === 'CH' ? 'CHF' : 'EUR',
                    })
                  }
                  options={countries}
                />
              ...
              <p className="fine">
                The interface is currently in English. Your preferred language is saved for future
                localisation.
              </p>
            </>
          )}
          ...
          {modal === 'listSettings' && (
            <label>
              ...
              <textarea
                ...
                onChange={(e) => setDraft({ ...draft, categoryOrder: e.target.value })}
              />
      ...
      >
        ...
        >
          <p className="fine">
            If another person cannot open Same Again, its owner needs to grant them Site access
            first. This household link cannot change that permission.
          </p>
          <label>
      ...
      <Modal open={modal === 'archives'} onClose={() => setModal('')} title="Archived lists">
        <div className="stack">
          ...
          {!s.records.some((r) => r.kind === 'list' && r.data.archived) && (
            ...
            (modal === 'history' ? trips : templates).map((t) => (
              <form
                ...
                onSubmit={(e) => {
                  ...
                  for (const i of selectedItems) add(t.data.items[i], undefined, true);
      ...
      <Modal
        open={modal === 'favourite'}
        onClose={() => setModal('')}
        title="Remember it your way."
      >
        <form
          className="stack"
          onSubmit={(e) => {
      ...
      >
        <form
          ...
          onSubmit={(e) => {
            e.preventDefault();
            s.mutate('feedback', { ...draft, user: s.user.id });
            setModal('product');
      ...
      >
        <form
          ...
          onSubmit={(e) => {
            e.preventDefault();
            s.mutate('observation', { ...draft, currency, user: s.user.id });
            setModal('');
      ...
      >
        <form
          ...
          onSubmit={async (e) => {
            ...
            const { name, constraintsText, categoriesText, ...rest } = draft;
        ...
        >
          ...
          <p className="fine">The interface currently uses English. Your preference is saved.</p>
          <label>
          ...
          <p className="fine">
            One per line: preference, exclusion, allergy or certification, followed by a colon and
            the ingredient or label. Use English catalogue terms. Free-text matching is conservative
            and cannot establish allergy safety. Always check the package.
          </p>
          <button
            disabled={
      ...
      >
        <div className="stack prose">
          ...
          <p>
            ...
            candidates with missing or conflicting required attributes. Shared ingredients,
            compatible per-100-unit nutrition, pack size and brand contribute to a reproducible
            score. The chosen priority changes their weight. Match bands summarise evidence; they
            are not a percentage of identicalness.
          </p>
          <p>
          ...
          <p>
            ...
            edits stop for review. Account deletion anonymises contributions retained in shared
            lists; household owners must transfer ownership or delete their household first.
          </p>
          <p>
          ...
          <p>
            Shared recipes keep photos, weighed ingredients and methods. Meal plans preserve a
            recipe snapshot. Nutrition uses matched per-100-g or per-100-ml values, scales by
            portions, and labels incomplete totals. Generic data is from the Swiss Food Composition
            Database, FSVO v7.1, reused with attribution under its published dataset permission.
            Generic composition values are averages and do not establish brand identity or allergy
            safety.
          </p>
          <p>
          ...
          <p>
            The interface is English; language preferences are saved. Product coverage is strongest
            for packaged food. Household essentials work as generic or private items. No retailer
            stock, verified taste equivalence, background reminders or full pantry tracking is
            connected. Family access also depends on the site's hosting access policy.
          </p>
      ...
      <Modal
        ...
        description="This is the same product, pack size, unit and note. Would you like to combine quantities?"
      >
        <p>{merge?.data.name}</p>
        <div className="row wrap">
          <button
            ...
            onClick={() => {
              s.mutate(
                ...
                {
                  ...merge.existing.data,
                  quantity: merge.existing.data.quantity + merge.data.quantity,
                },
                merge.existing,
              );
              setMerge(null);
      ...
      >
        <div className="grid2">
          ...
          <div>
            ...
            {s.conflict?.op.kind !== 'item' && (
              <pre className="conflict-data">
                {JSON.stringify(s.conflict?.latest?.data, null, 2)}
              </pre>
            )}
          </div>
        </div>
