...
export async function retailerCatalogue(retailer: string, q: string, page = 1) {
  ...
  const where =
    "json_extract(data,'$.retailer')=? AND (json_extract(data,'$.name') LIKE ? ESCAPE '\\' OR json_extract(data,'$.brand') LIKE ? ESCAPE '\\')";
  const total = await one(
    'SELECT count(*) AS n FROM catalogue WHERE ' + where,
    retailer,
    term,
    term,
  );
  const rows = await query(
    'SELECT data FROM catalogue WHERE ' +
      where +
      " ORDER BY CASE WHEN json_extract(data,'$.evidence')='retailer-page' THEN 0 ELSE 1 END,json_extract(data,'$.name'),id LIMIT 24 OFFSET ?",
    retailer,
    term,
    term,
    (page - 1) * 24,
  ...
  return {
    products: rows.map((r) => JSON.parse(r.data) as Product),
    total: total.n,
    page,
    hasMore: page * 24 < total.n,
    coverage: {
      complete: false,
      imported: products.filter((p) => p.retailer === retailer).length,
      pageDetails: products.filter((p) => p.retailer === retailer && p.evidence === 'retailer-page')
        .length,
    },
    notice:
      'Partial import. Indexed titles are discovery links, not a current retailer assortment. Page details show their retrieval date. Stock and prices are not verified.',
  };
}
