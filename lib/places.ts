...
export function places(data: any, origin?: Point): Place[] {
  ...
  for (const f of data.features) {
    ...
    let at: Point;
    try {
      at = point(coords[1], coords[0]);
    } catch {
      continue;
    }
    const id = type + '/' + p.osm_id;
    if (seen.has(id)) continue;
    seen.add(id);
    const km = origin ? distance(origin, at) : undefined;
    if (km != null && km > 3.2) continue;
    result.push({
      ...at,
      id,
      name: p.name || 'Unnamed ' + (p.osm_value || 'place'),
      address: [p.housenumber, p.street, p.postcode, p.city || p.district, p.state]
        .filter(Boolean)
        .join(' '),
      country: p.country || '',
      countryCode: typeof p.countrycode === 'string' ? p.countrycode.toUpperCase() : undefined,
      type: p.osm_value || 'place',
      sourceUrl: 'https://www.openstreetmap.org/' + id,
      ...(km == null ? {} : { distance: Math.round(km * 10) / 10 }),
    });
  }
  return result.sort((a, b) => (a.distance || 0) - (b.distance || 0));
}
export function placesUrl(base: string, at?: Point, q?: string) {
  const url = new URL(at ? 'reverse' : 'api/', base.endsWith('/') ? base : base + '/');
  if (at) {
  ...
  } else url.search = new URLSearchParams({ q: q || '', limit: '5' }).toString();
  return url.toString();
}
