...
export async function swissSearch(q:string,country?:string,category?:string,store?:string,requestUrl?:string){return rankSearch((await index(requestUrl)).products.filter(p=>!store||p.stores?.some(s=>s.toLowerCase()===store)),q,{country,category})}
