  ...
  if (!loaded)
    loaded = (async () => {
      const r = await (env as any).ASSETS.fetch(
        new Request(
          new URL(
            ...
            requestUrl || 'https://same-again.frx.chatgpt.site',
          ),
        ),
      );
      ...
      return {
        ...
        byBarcode: new Map(
          products
            .filter((p) => p.barcode && barcode(p.barcode).valid)
            .map((p) => [barcode(p.barcode!).code, p]),
        ),
      };
    })().catch((e) => {
      loaded = undefined;
...
export const swissBarcode = async (code: string, requestUrl?: string) =>
  (await index(requestUrl)).byBarcode.get(code);
export async function swissSearch(
  ...
  return rankSearch(
    (await index(requestUrl)).products.filter(
      (p) => !store || p.stores?.some((s) => s.toLowerCase() === store),
    ),
    q,
    { country, category },
  );
