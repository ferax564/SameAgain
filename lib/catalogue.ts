...
export const off:CatalogueProvider={async lookup(raw){const b=barcode(raw);if(!b.valid)fail(b.error);const data=await cached('product-details-v2:'+b.code,'https://world.openfoodfacts.org/api/v3.6/product/'+b.code+'.json?fields='+fields,'product');if(!data?.product)return null;return persist({...normalise(data.product,data._sameAgainRetrieved),detailsRetrieved:data._sameAgainRetrieved})},async search(q,country,category,store){const url=searchUrl(q,country,category,store);const data=await cached('search-v2:'+url,url,'search');const products=data.hits.filter((p:any)=>p.code&&(p.product_name||p.product_name_en)).map((p:any)=>normalise(p,data._sameAgainRetrieved)).filter((p:Product)=>(!country||p.countries.includes(countryTag(country)))&&(!category||p.categories.includes(category))&&(!store||p.stores?.some(s=>s.toLowerCase()===store)));const saved=[];for(const p of products)saved.push(await persist(p));return saved}};
