...
export interface CatalogueProvider {
  ...
  search(q: string, country?: string, category?: string, store?: string): Promise<Product[]>;
}
  ...
  if (
    ...
    for (const [key, meta] of Object.entries(nutrients)) {
      ...
      if (
        ...
        continue;
      if (v.unit === meta.unit) n[key] = v.value;
      else if (grams[v.unit] && grams[meta.unit])
        n[key] = (v.value * grams[v.unit]) / grams[meta.unit];
    }
  } else
    for (const k of [
      'energy-kcal',
      ...
      if (
        typeof p.nutriments?.[k + '_100g'] === 'number' &&
        Number.isFinite(p.nutriments[k + '_100g']) &&
        p.nutriments[k + '_100g'] >= 0
      )
        n[k] = p.nutriments[k + '_100g'];
  return {
    ...
    ingredients:
      ...
      undefined,
    ingredientTags: p.ingredients?.map((i: any) => i.id).filter(Boolean),
...
async function cached(key: string, url: string, type: 'product' | 'search') {
  ...
  try {
    ...
    if (type === 'search' && (!Array.isArray(data.hits) || data.timed_out))
...
async function persist(p: Product) {
  await run(
    "INSERT INTO catalogue(id,data,retrieved,search_text) VALUES(?,?,?,?) ON CONFLICT(id) DO UPDATE SET data=excluded.data,retrieved=excluded.retrieved,search_text=excluded.search_text WHERE COALESCE(json_extract(excluded.data,'$.detailsRetrieved'),0)>COALESCE(json_extract(catalogue.data,'$.detailsRetrieved'),0) OR (COALESCE(json_extract(catalogue.data,'$.detailsRetrieved'),0)=0 AND excluded.retrieved>=catalogue.retrieved)",
    p.id,
    JSON.stringify(p),
    p.retrieved,
    productSearchText(p),
...
export const off: CatalogueProvider = {
  ...
  async search(q, country, category, store) {
    const url = searchUrl(q, country, category, store);
    const data = await cached('search-v2:' + url, url, 'search');
    const products = data.hits
      .filter((p: any) => p.code && (p.product_name || p.product_name_en))
      .map((p: any) => normalise(p, data._sameAgainRetrieved))
      .filter(
        (p: Product) =>
