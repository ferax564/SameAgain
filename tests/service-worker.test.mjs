...
function runtime() {
  ...
  const caches = {
    ...
    async match(k) {
      return stored.get(key(k))?.clone();
    },
    async keys() {
      return ['same-again-shell-v1', 'unrelated-cache'];
    },
    async delete(k) {
      stored.set('deleted:' + k, true);
    },
  };
  vm.runInNewContext(code, {
    ...
    AbortSignal,
    caches,
    location: { origin: 'https://same.test' },
    fetch: (...a) => network(...a),
    self: {
...
test('API and sign-in requests are never intercepted or cached', async () => {
  const r = runtime();
  for (const path of ['/api/data', '/api/photo', '/signin-with-chatgpt', '/callback'])
    assert.equal(await r.dispatch({ ...nav, url: 'https://same.test' + path }), undefined);
});
test('access-gate HTML cannot replace the application shell', async () => {
  const r = runtime();
  r.stored.set('/', new Response('app shell'));
  r.setNetwork(async () => new Response('Sign in'));
  await r.dispatch(nav);
  assert.equal(await r.stored.get('/').text(), 'app shell');
});
test('cache upgrade removes only Same Again caches', async () => {
  const r = runtime();
  await r.event('activate');
  assert.equal(r.stored.get('deleted:same-again-shell-v1'), true);
  assert(!r.stored.has('deleted:unrelated-cache'));
});
